...
# imports 
def import_image(*path, alpha=True, format='png', scale_factor=1):
    full_path = join(*path) + f'.{format}'
    surf = pygame.image.load(full_path).convert_alpha() if alpha else pygame.image.load(full_path).convert()

    if scale_factor != 1:
        width, height = surf.get_size()
        surf = pygame.transform.scale(surf, (int(width * scale_factor), int(height * scale_factor)))
...
def import_tilemap(cols, rows, *path,scale_factor = 1):
	frames = {}
	surf = import_image(*path,scale_factor=scale_factor)
...
def new_character_importer(cols,rows,*path):
	frame_dict = import_tilemap(cols,rows,*path,scale_factor=0.8)
	new_dict = {}
...
def all_character_import(*path):
	...
	for _, __, image_names in walk(join(*path)):
		for image in image_names:
			...
			if image_name == 'mc_mini' or 'seller':
				new_dict[image_name] = new_character_importer(9,4,*path,image_name)
			else:
